import discord
import random
from config.error_handling.getError import getErrMsg
...
def rollDie(amt, diesize, bonus, mod, raw):
    ...
    if "kh" in mod:
        ...
        for i in range(0, len(rolls)):
            if bonus != 0:
                rollsString += (
                    f"#{i}: {rolls[i]} ({(rolls[i]) - bonus}{bonusSign}{bonus})\n"
                )
            elif bonus == 0:
    ...
    if raw:
        ...
        if len(rolls) <= 1024:
            ...
            errEmbed = discord.Embed(
                title="Error",
                description="I'm sorry, but that roll generates a message longer than 1024 characters, which is longer than Discord's max embed message length. Please try again and reduce your roll amount.",
                color=0xFF0000,
            )
            return errEmbed
    ...
    try:
        if len(split) == 5:
            if rig:
                ...
                return rollDie(
                    int(diesize[0]), int(diesize[1]), split[3], split[4], rawroll
                )
        elif len(split) == 4:
            if isbonus(split[3]):
                if rig:
                    ...
                    return rollDie(
                        int(diesize[0]), int(diesize[1]), split[3], "None", rawroll
                    )
            else:
                if rig:
                    return rollDie(
                    ...
                    return rollDie(
                        int(diesize[0]), int(diesize[1]), 0, split[3], rawroll
                    )
        elif len(split) == 3:
            if rig:
                return rollDie(
